...
res_fname = "NetPosSwitchingResults/POSB_"+model+"_N"+str(int(n))+"_p"+str(int(p*100))+"_bch"+str(swt_batch_count)+".pkl"
print(res_fname)
...
if not os.path.isfile(res_fname):
    ...
    while True:
        ret = S.switch_A(alg='BLOC', count=swt_batch_count)
        rBest.append((S.swt_done, S.assortativity_coeff()))
        if ret != -1:
            break
    plt.figure()
    plt.plot([i[0] for i in rBest], [i[1] for i in rBest])
    #plt.imshow(S.A)
    plt.show()
    1/0
    print('BEST done')
